...
1. This is done in dataSet1.R file with the following code:mytickers = c("SPY", "AAPL", "YHOO")  
...
1. We can do that by changing the startDate = "2001-01-01" in dataSet1.R file
<br>
<br>
<br>
...
`r write.csv(StockReturns[1:20,c("SPY","AAPL")], file = "twentydays.csv", row.names = TRUE, col.names = TRUE) 
myData <- read.csv(file = "twentydays.csv", header = TRUE, sep=";")
sum(myData != StockReturns[1:20,])
`

2. There is a missing parentheses in this command so an error appears.
